...
internal struct PackageVersionPlugin: BuildToolPlugin {
  private func GitRevision() throws -> String {
    let git: String? = withUnsafeTemporaryAllocation(of: WCHAR.self, capacity: Int(MAX_PATH)) { pBuffer in
      "git.exe".withCString(encodedAs: UTF16.self) {
        _ = wcscpy_s(pBuffer.baseAddress, Int(pBuffer.count), $0)
      }
      guard PathFindOnPathW(pBuffer.baseAddress, nil) else { return nil }
      return String(decodingCString: pBuffer.baseAddress!, as: UTF16.self)
    }

    guard let git else { throw PluginError("`git.exe` not found") }
  ...
  func createBuildCommands(context: PluginContext, target: Target) throws -> [Command] {
    ...
    internal var version: String { "\(try GitRevision())" }
